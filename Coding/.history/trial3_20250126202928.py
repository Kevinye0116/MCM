...
class TourismOptimization:
    ...
    def calculate_social_satisfaction(self, x: np.ndarray) -> float:
        ...
        # S = k4*Sresidents
        return self.k4 * np.clip(S_residents, 0, 100)
    ...
    def calculate_environmental_quality(self, x: np.ndarray) -> float:
        ...
        return (
            self.k1 * (CO2p_adj * Nt - self.Cbase)  # 添加Cbase项
            + self.k2 * Nt / Cwaste_adj
            + self.k3 * Nt / Cwater_adj
        )

    def calculate_capacity_changes(self, x: np.ndarray) -> Tuple[float, float, float]:
        """计算容量变化率 (公式4)"""
        P_waste, P_water, P_e = self.calculate_investments(x)

        dCwaste_dt = self.alpha1 * P_waste
        dCwater_dt = self.alpha2 * P_water
        dCbase_dt = self.alpha3 * P_e

        return dCwaste_dt, dCwater_dt, dCbase_dt

    def calculate_investments(self, x: np.ndarray) -> Tuple[float, float, float]:
        """计算投资金额 (公式5)"""
        Nt, tau_t = x
        Re = self.calculate_revenue(x)

        # 投资计算
        P_waste = self.k5 * tau_t * Re
        P_water = self.k6 * tau_t * Re
        P_e = self.k7 * tau_t * Re
    ...
    def calculate_objective(self, x: np.ndarray) -> float:
        """计算目标函数 Z = Re + S - E (公式6)"""
        Re = self.calculate_revenue(x)
        S = self.calculate_social_satisfaction(x)
        E = self.calculate_environmental_quality(x)
        return Re + S - E
    ...
    def constraints(self, x: np.ndarray) -> List[float]:
        ...
        constraints = [
            Re,  # Re ≥ 0
            0.08 - tau_t,  # τt ≤ 8%
            self.Pt_max - self.Pt,  # Pt ≤ Ptmax
            self.Nt_max - Nt,  # 0 ≤ Nt ≤ Ntmax
            Nt,  # Nt ≥ 0
            self.CO2_max - Nt * CO2p_adj,  # Nt·CO2p ≤ CO2max
            (1.2 / 365) * Cwaste_adj - 0.012 * Nt,  # 0.012Nt ≤ (1.2/365)Cwaste
            (1.2 / 365) * Cwater_adj - 0.012 * Nt,  # 0.012Nt ≤ (1.2/365)Cwater
            self.calculate_social_satisfaction(x) - 60,  # Sresidents ≥ 60
        ]
        return constraints
