...
class EnhancedTourismOptimizer:
    ...
    def objective_de(self, x):
        ...
        penalty = self.adaptive_penalty(x)

        return -(Re + S - E) + penalty

    def objective_local(self, x):
        """Objective function for local optimization"""
        return self.objective_de(x)
    ...
    def hybrid_optimize(self, de_popsize=20, de_maxiter=30, local_maxiter=1000):
        ...
        # Set iteration parameters for adaptive penalty
        self.max_iterations = de_maxiter

        # Phase 1: Differential Evolution
        de_result = differential_evolution(
            self.objective_de,  # Using class method instead of lambda
            self.bounds,
            popsize=de_popsize,
            maxiter=de_maxiter,
            strategy="best1bin",
        ...
        # Update parameters for local optimization
        self.max_iterations = local_maxiter
        ...
        for i, pert in enumerate([np.zeros_like(base_point)] + perturbations):
            ...
            result = minimize(
                self.objective_local,  # Using class method instead of lambda
                start_point,
                method="SLSQP",
                bounds=self.bounds,
