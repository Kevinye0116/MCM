...
class TourismOptimizer:
    ...
    def constraints(self):
        ...
        cons = [
            ...
            {
                "type": "ineq",
                "fun": lambda x: x[2] - 0.05,
            },  # k5 >= 0.05 确保最小投资比例
            {
                "type": "ineq",
                "fun": lambda x: x[3] - 0.05,
            },  # k6 >= 0.05 确保最小投资比例
            {
                "type": "ineq",
                "fun": lambda x: x[4] - 0.05,
            },  # k7 >= 0.05 确保最小投资比例
        ]
        return cons
    ...
    def optimize(self):
        # 调整初始猜测值，使其更合理
        x0 = [100000, 0.05, 0.13, 0.13, 0.13]  # [Nt, tau_t, k5, k6, k7]
    ...
    def check_constraints(self, x):
        ...
        print(f"Revenue (Re): {Re}")
        print(f"CO2 emissions: {Nt * self.CO2p} (max: {self.CO2max})")
        print(f"Waste usage: {0.012 * Nt} (capacity: {1.2/365 * C_waste_final})")
        print(f"Water usage: {0.012 * Nt} (capacity: {1.2/365 * C_water_final})")
        print(f"Resident satisfaction: {Sresidents} (min: 60)")
        print(f"Investment ratio sum: {k5 + k6 + k7} (max: 0.4)")
