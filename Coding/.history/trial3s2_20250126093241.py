...
from pymoo.util.hv import HyperVolume  # 修正类名大小写
from pymoo.util.ref_dirs import get_reference_directions  # 超体积计算支持
from SALib import analyze, sample  # 新增SALib库
...
class TourismOptimization:
    ...
    def calculate_hypervolume(
        ...
        objectives = np.array(
            [
                [
                    -self.calculate_revenue(x),  # 转换为最小化问题
                    -self.calculate_social_satisfaction(x),  # 转换为最小化问题
                    -self.calculate_environmental_quality(x),  # 转换为最小化问题
                ]
                for x in pareto_front
            ]
        )
        ...
        if ref_point is None:
            ref_point = np.max(objectives, axis=0) * 1.1  # 使用自动计算的参考点

        # 使用pymoo的超体积计算（注意参数顺序）
        hv_calculator = HyperVolume()
        return hv_calculator.do(objectives, ref_point)


def sensitivity_analysis(
    optimizer: TourismOptimization,
    params_to_analyze: list = ["k1", "k2", "k3"],
    population_size: int = 50,
    generations: int = 30,
    n_samples: int = 128,  # 改为2的幂次方
):
    """执行全局敏感性分析"""
    # 定义参数空间
    problem = {
        "num_vars": len(params_to_analyze),
        "names": params_to_analyze,
        "bounds": [
            [0.5 * getattr(optimizer, p), 1.5 * getattr(optimizer, p)]
    ...
    param_values = sobol.sample(
        problem, n_samples, skip_values=1024
    )  # 添加skip_values参数

    # 存储超体积结果
    hypervolumes = []

    print(f"\nStarting sensitivity analysis with {len(param_values)} samples...")
    for i, params in enumerate(param_values):
        # 更新模型参数
        optimizer.update_parameters(dict(zip(problem["names"], params)))
        ...
        # 计算超体积指标
        hv = optimizer.calculate_hypervolume(pareto_front)
        hypervolumes.append(hv)

        print(f"Sample {i+1}/{len(param_values)} completed. Hypervolume: {hv:.2f}")

    # 执行敏感性分析
    si = analyze.sobol.analyze(problem, np.array(hypervolumes), print_to_console=False)

    # 打印结果
    print("\nSensitivity Indices:")
    print(f"{'Parameter':<10} | {'Main Effect':<12} | {'Total Effect':<12}")
    for name, s1, st in zip(problem["names"], si["S1"], si["ST"]):
        print(
            f"{name:<10} | {s1:.4f}{'*' if s1 > 0.1 else '':<11} | {st:.4f}{'*' if st > 0.2 else '':<11}"
        )
