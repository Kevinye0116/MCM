...
def setup_toolbox():
 ...
 toolbox.register("attr_n_tourists", np.random.uniform, N_TOURISTS_MIN, N_TOURISTS_MAX)
    toolbox.register("attr_tax", np.random.uniform, TAX_MIN, TAX_MAX)
    toolbox.register("attr_invest_eco", np.random.uniform, INVEST_ECO_MIN, INVEST_ECO_MAX)

    # 使用initCycle组合三个独立变量
    toolbox.register("individual", tools.initCycle, creator.Individual,
                     (toolbox.attr_n_tourists,
                      toolbox.attr_tax,
                      toolbox.attr_invest_eco),
                     n=1)
    # 注册变量和种群
