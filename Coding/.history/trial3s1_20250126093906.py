...
class TourismOptimization:
        ...
        for idx, param_name in enumerate(parameter_names):
            ...
            with ProcessPoolExecutor() as executor:
                ...
                for val in variations_values:
                    futures.append(
                        executor.submit(
                            self._analyze_single_variation,
                            param_name,
                            val,
                            original_value,
                        )
    ...
    def _analyze_single_variation(self, param_name, val, original_value):
        setattr(self, param_name, val)
        try:
            pareto_front = self.pareto_optimization(population_size=50, generations=20)
            if len(pareto_front) > 0:
                obj_values = np.array(
                    [
                        [
                ...
                # 使用中位数代替平均值
                revenues = (
                    np.median(obj_values[:, 0]) if len(obj_values) > 0 else np.nan
                )
                satisfactions = (
                    np.median(obj_values[:, 1]) if len(obj_values) > 0 else np.nan
                )
                environments = (
                    np.median(obj_values[:, 2]) if len(obj_values) > 0 else np.nan
                )
                return revenues, satisfactions, environments
            else:
