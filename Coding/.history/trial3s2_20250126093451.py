...
class TourismOptimization:
    ...
    def calculate_hypervolume(
        ...
        if ref_point is None:
            ref_point = np.max(objectives, axis=0) * 1.1  # 自动计算参考点
            ref_point = np.maximum(ref_point, 1e-6)  # 确保参考点不为零

        # 使用pymoo的超体积计算（正确初始化方式）
        hv_calculator = HyperVolume(reference_point=ref_point)  # 添加参考点参数
        return hv_calculator.do(objectives)
