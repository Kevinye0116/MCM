...
class ImprovedTourismOptimizer1:
    ...
    def constraint_violation_penalty(self, x):
        ...
        CO2max = self.CO2max + self.alpha1 * P[2]
        ...
        if co2_emissions > CO2max:
            penalty += (co2_emissions - CO2max) ** 2

        # Carrier constraints
        if 0.012 * Nt > 1.2 * Cwaste / 365:
            penalty += (0.012 * Nt - 1.2 * Cwaste / 365) ** 2

        if 0.012 * Nt > 1.2 * Cwater / 365:
            penalty += (0.012 * Nt - 1.2 * Cwater / 365) ** 2
...
def main():
    optimizer = ImprovedTourismOptimizer1()
